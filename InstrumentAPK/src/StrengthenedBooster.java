...
public class StrengthenedBooster {
	...
	public static void reflectionAnalysis()
	{
		...
		Map<StmtKey,StmtValue> stmtKeyValues = MessageResolver.extractStmtKeyValue(elements);
		for(Map.Entry<StmtKey, StmtValue> entry : stmtKeyValues.entrySet()){
			if(DroidRAResult.stmtKeyValues.containsKey(entry.getKey())){
				StmtValue resultValue = DroidRAResult.stmtKeyValues.get(entry.getKey());
				Set<ClassDescription> clsSet = resultValue.getClsSet();
				clsSet.addAll(entry.getValue().getClsSet());
			}else{
				DroidRAResult.stmtKeyValues.put(entry.getKey(), entry.getValue());
			}
		}
